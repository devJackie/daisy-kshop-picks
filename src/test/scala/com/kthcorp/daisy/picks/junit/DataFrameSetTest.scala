    ...
    def main(args: Array[String]): Unit = {
        ...
        dfWithoutSchema.foreachPartition((rdd: Iterator[Row]) => {
            rdd.foreach(x => {
                ...
                println(x.getAs[Row]("recommends"))
                println(s">>>>>>>>>>>>x: $x")
                val recommends = x.getAs[mutable.WrappedArray[mutable.WrappedArray[Double]]]("recommends")
                
                println(s">>>>>>>>>>>>recommends: $recommends")
                println(recommends(0))
                println(recommends(1))
    
                recommends.foreach { array =>
                    println(s">>>>>>>>>>>>array: $array")
                    val Array(item, prediction) = array.toArray
...
//            val rowValues = ("-2054210684", 394924, 0.06045123)
//            val rowValues1 = spark.sparkContext.parallelize(
//                Seq(
//                    ("-2054210684", 394924, 0.06045123))
//            )
////            val rdd1 = spark.sparkContext.parallelize(rowValues1)
//
//            val df2 = spark.createDataFrame(rowValues1).toDF("user", "item", "prediction")
//            df2.show()
        })
